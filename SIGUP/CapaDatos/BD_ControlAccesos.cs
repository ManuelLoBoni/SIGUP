...
namespace CapaDatos
{
    public class BD_ControlAccesos
    {
        public List<EN_ControlAccesos> ListarAccesosCompleto()
        {
            ...
            try
            {
                using (SqlConnection oConexion = new SqlConnection(BD_Conexion.cn))
                {
                    ...
                    sb.AppendLine("SELECT CA.IdRegistro, CONVERT(char(10), CA.fecha, 103)Fecha, Us.IdUsuario, CONCAT(Us.Nombre,' ',Us.Apellidos)Usuario, CA.HoraEntrada,");
                    sb.AppendLine("CA.HoraSalida, TA.IdActividad, TA.NombreActividad, CA.CantidadAlumnos AS Alumnos,");
                    sb.AppendLine("CA.Semestre,Cr.IdCarrera,Cr.NombreCarrera AS Carrera,a.IdArea,a.NombreArea as Area");
                    sb.AppendLine("from ControlAccesos CA");
                    sb.AppendLine("INNER JOIN TipoActividad TA ON TA.IdActividad = CA.TipoActividad");
                    sb.AppendLine("INNER JOIN usuario Us ON Us.IdUsuario = CA.IdUsuario");
                    sb.AppendLine("INNER JOIN Carreras Cr ON CA.IdCarrera = Cr.IdCarrera");
                    sb.AppendLine("INNER JOIN Areas a on a.IdArea = CA.Area order by CA.IdRegistro DESC");
        ...
        public List<EN_ControlAccesos> ListarAccesosParaBitacora()
        {
            ...
            try
            {
                using (SqlConnection oConexion = new SqlConnection(BD_Conexion.cn))
                {
                    ...
                    sb.AppendLine("SELECT CA.IdRegistro, CONVERT(char(10), CA.fecha, 103) AS Fecha, Us.IdUsuario, CONCAT(Us.Nombre,' ',Us.Apellidos) AS Usuario,");
                    sb.AppendLine("CA.HoraEntrada, CA.HoraSalida, TA.IdActividad, TA.NombreActividad, CA.CantidadAlumnos AS Alumnos,");
                    sb.AppendLine("CA.Semestre, Cr.IdCarrera, Cr.NombreCarrera AS Carrera, a.IdArea, a.NombreArea AS Area");
                    sb.AppendLine("from ControlAccesos CA");
                    sb.AppendLine("INNER JOIN TipoActividad TA ON TA.IdActividad = CA.TipoActividad");
                    sb.AppendLine("INNER JOIN usuario Us ON Us.IdUsuario = CA.IdUsuario");
                    sb.AppendLine("INNER JOIN Carreras Cr ON CA.IdCarrera = Cr.IdCarrera");
                    sb.AppendLine("INNER JOIN Areas a on a.IdArea = CA.Area WHERE TA.IdActividad = 1");
                    sb.AppendLine("AND NOT EXISTS (SELECT 1 FROM Bitacora B WHERE CA.IdRegistro = B.IdRegistro)");
                    sb.AppendLine("ORDER BY CA.IdRegistro DESC");
        ...
        public List<EN_ControlAccesos> ListarEditarParaBitacora()
        {
            List<EN_ControlAccesos> lista = new List<EN_ControlAccesos>();
            try
            {
                using (SqlConnection oConexion = new SqlConnection(BD_Conexion.cn))
                {
                    ...
                    sb.AppendLine("INNER JOIN usuario Us ON Us.IdUsuario = CA.IdUsuario");
                    sb.AppendLine("INNER JOIN Carreras Cr ON CA.IdCarrera = Cr.IdCarrera");
                    sb.AppendLine("INNER JOIN Areas a on a.IdArea = CA.Area where TA.IdActividad = 1 order by CA.IdRegistro DESC");
        ...
        public int GuardarRCU(EN_ControlAccesos registroCU, out string Mensaje)
        {
            ...
            try
            {
                using (SqlConnection oConexion = new SqlConnection(BD_Conexion.cn))
                {
                    SqlCommand cmd = new SqlCommand("sp_RegistrarCA", oConexion);
        ...
        public bool ElimnarRegistroCU(int id, out string Mensaje)
        {
            ...
            try
            {
                using (SqlConnection oConexion = new SqlConnection(BD_Conexion.cn))
                {
                    SqlCommand cmd = new SqlCommand("sp_EliminarCA", oConexion);
        ...
        public bool EditarRCU(EN_ControlAccesos registroCU, out string Mensaje)
        {
            ...
            try
            {
                using (SqlConnection oConexion = new SqlConnection(BD_Conexion.cn))
                {
                    SqlCommand cmd = new SqlCommand("sp_EditarCA", oConexion);
        ...
        public bool SalidaRCU(EN_ControlAccesos registroCU, out string Mensaje)
        {
            ...
            try
            {
                using (SqlConnection oConexion = new SqlConnection(BD_Conexion.cn))
                {
                    SqlCommand cmd = new SqlCommand("sp_SalidaCA", oConexion);
                    cmd.Parameters.AddWithValue("@IdRegistro", registroCU.IdRegistro);
                    cmd.Parameters.AddWithValue("@HoraSalida", registroCU.HoraSalida);
                    //Dos parametros de salida, un entero de resultaado y un string de mensaje
                    cmd.Parameters.Add("@Resultado", SqlDbType.Bit).Direction = ParameterDirection.Output;
